...
import { analyzeCustomPropertyData, type CustomSchemaConfig } from "@/lib/customLLM";
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Failed to process test scraping",
      },
      { status: 500 }
    );
  }
...
async function handleStreamingRequest(urls: string[], config: CustomSchemaConfig) {
...
async function handleTraditionalRequest(urls: string[], config: CustomSchemaConfig) {
  console.log(`Starting test scraping for ${urls.length} URLs`);
  
  const results: ScrapingResult[] = [];

  // Process URLs one by one (limit to 5 for testing)
  const urlsToProcess = urls.slice(0, 5);
