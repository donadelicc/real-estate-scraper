...
import { ChevronRight, ChevronDown, Link, Globe, Folder, FileText, Eye, EyeOff } from 'lucide-react'
...
export default function HTMLThree({ 
  ...
  const treeData = useMemo(() => {
    ...
    const filteredLinks = showOnlySelected && selectedCategories.length > 0
      ? links.filter(link => {
  ...
  const renderNode = (node: URLNode): React.ReactNode => {
    ...
    const isSelected = node.category && selectedCategories.includes(node.category)
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2">
              <span className={`text-sm font-medium truncate ${getCategoryColor(node.category)}`}>
                {node.name}
              </span>
              {node.category && (
                <span className={`text-xs px-2 py-1 rounded-full bg-gray-100 ${getCategoryColor(node.category)}`}>
  ...
  const visibleUrls = showOnlySelected && selectedCategories.length > 0
    ? links.filter(link => {
        for (const [category, info] of Object.entries(urlCategories)) {
          if (info.examples.includes(link) && selectedCategories.includes(category)) {
            return true
          }
        }
        return false
    ...
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
      ...
      <div className="bg-gray-50 border-b border-gray-200 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-800">Website Structure</h3>
              <p className="text-sm text-gray-600">
                {visibleUrls} of {totalUrls} URLs
                {showOnlySelected && selectedCategories.length > 0 && (
                  <span className="ml-1 text-blue-600">(filtered by selected categories)</span>
                )}
              </p>
            </div>
          </div>
      ...
      {Object.keys(urlCategories).length > 0 && (
        <div className="bg-gray-50 border-t border-gray-200 p-4">
          <h4 className="text-sm font-medium text-gray-800 mb-3">URL Categories</h4>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {Object.entries(urlCategories).map(([category, info]) => (
